...
# Hyper Parameters
EPOCHs = 2          # train the training data n times, to save time, we just train 1 epoch
BATCH_SIZE = 50
...
class CNN(nn.Module):
    def __init__(self):
        ...
        # )
        self.out = nn.Linear(16 * 28 *28, 10)   # fully connected layer, output 10 classes
...
accuracy_list = []
epoch_list = []
name_file = '30-leakyrelucnn-mnist1'
epoch=1
    ...
    for step, (b_x, b_y) in enumerate(train_loader):   # gives batch data, normalize x when iterate train_loader
        ...
        if step % 50 == 0:
            ...
            file_w.write('\n' + '[epoch %d]  train_loss: %.3f  val_accuracy: %.3f  train_time: %.2f s' %
                         (epoch, loss.data.numpy(), accuracy, (t2 - t1)))
            file_w.close()
            epoch = epoch + 1
            if HAS_SK:
